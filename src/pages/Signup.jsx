  ...
  return (
    <AuthLayout>
      <div className="rounded-xl p-7" style={{ background: 'var(--bg2)', border: '1px solid var(--border)' }}>
        {!done ? (
          <>
            <div className="text-lg font-semibold mb-1" style={{ color: 'var(--text)' }}>Request access</div>
            <div className="text-xs font-mono mb-5 leading-relaxed" style={{ color: 'var(--text3)' }}>
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              {fields.map(f => (
                <div key={f.id}>
                  <label className="block text-xs uppercase tracking-wider mb-1.5" style={{ color: 'var(--text3)' }}>{f.label}</label>
                  <input
                    type={f.type} value={f.val} onChange={e => f.set(e.target.value)}
                    placeholder={f.ph} className={inputBase}
                  />
                </div>
              ))}
          ...
          <div className="text-center py-4">
            <div className="text-4xl mb-4 font-mono" style={{ color: 'var(--accent)' }}>[ok]</div>
            <div className="text-base font-semibold mb-2" style={{ color: 'var(--text)' }}>Request submitted</div>
            <div className="text-xs font-mono leading-relaxed" style={{ color: 'var(--text3)' }}>
      ...
      <div className="text-center mt-5 text-xs font-mono" style={{ color: 'var(--text3)' }}>
        ...
        <Link to="/login" className="cm-link transition-colors">
          Sign in
        </Link>
      </div>
    </AuthLayout>
